...
const PendingOrders = () => {
  ...
  const filteredOrders = pendingOrders.filter(
    (order) => order.product_name.toLowerCase().includes(searchQuery.toLowerCase() || order.email.toLowerCase().includes(searchQuery.toLowerCase())
  ))
                        ...
                        <div className="flex items-center justify-center gap-1">
                          {product?.status === "pending" ? (
                            <>
                              ...
                              {/* Reject */}
                              <button onClick={()=> handleReject(product._id)} className="flex items-center space-x-2 bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-2 rounded-lg transition-all shadow-md hover:shadow-lg font-medium">
                                <X className="w-4 h-4" />
                              </button>
                            </>
